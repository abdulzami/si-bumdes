...
@section('content-header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Manajemen <span class="badge badge-warning">Hutang</span></h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('login') }}">Home</a></li>
                    <li class="breadcrumb-item active">Manajemen Hutang</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
...
@section('content')
    <div class="container-fluid">
        <a href="{{ route('hutang.create') }}" class="btn btn-outline-dark mb-3"><i class="fa fa-plus ">
        </i> Tambah</a>
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Data Hutang
                        </h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach ($hutangs as $index => $hutang)
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                $i = Crypt::encryptString($hutang->id);
                                                @endphp
                                                @if ($hutang->status_hutang == "belum lunas")
                                                <a href="#" data-id="{{$i}}" class="btn btn-sm btn-outline-dark mb-3 lunas">
                                                    <form action="{{route('hutang.lunas',$i)}}" method="POST" id="lunas{{$i}}">
                                                        @csrf
                                                    </form><i class="fa fa-check "></i> Lunas
                                                </a>
                                                <a href="{{route('hutang.edit',$i)}}"
                                                    class="btn btn-sm btn-outline-primary mb-3">
                                                    <i class="fa fa-edit ">
                                                    </i> Edit
                                                </a>
                                                <a href="#" data-id="{{$i}}" class="btn btn-sm btn-outline-danger mb-3 swall-yeah">
                                                    <form action="{{route('hutang.delete',$i)}}" method="POST" id="delete{{$i}}">
                                                        @csrf
                                                        @method('delete')
